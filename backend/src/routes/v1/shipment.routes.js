...
// Create shipment (staff+ only) - Protected by Idempotency
router.post('/', authorizeMinRole(USER_ROLES.STAFF), validate(shipmentValidator.createShipment), idempotencyMiddleware, shipmentController.createShipment);

// Get single shipment by UUID
router.get('/:uuid', validate(shipmentValidator.uuidParam), shipmentController.getShipmentByUuid);

// Update shipment (staff+ only) - Protected by Idempotency
router.put('/:uuid', authorizeMinRole(USER_ROLES.STAFF), validate(shipmentValidator.updateShipment), idempotencyMiddleware, shipmentController.updateShipment);
